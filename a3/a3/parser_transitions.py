...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0 :
        ...
        for par,transition in zip(parses,transitions):
            # print(transition)
            par.parse([transition])
            # print(par.dependencies)
            if len(par.buffer)==0 and len(par.stack)==1:
                unfinished_parses.remove(par)
