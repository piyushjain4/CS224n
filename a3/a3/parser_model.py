...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(w1,gain=nn.init.calculate_gain('relu')))
        ...
        self.hidden_to_logits_weight = nn.Parameter(nn.init.xavier_uniform_(w2,gain=nn.init.calculate_gain('relu')))
    ...
    def embedding_lookup(self, w):
        ...
        b,_ = w.shape
        xs = []
        x = torch.zeros((b,self.n_features,self.embed_size),dtype=torch.long)
        for feat_idxs in w :
            embed = torch.index_select(self.embeddings,0,feat_idxs)
            xs.append(embed)
        x = torch.stack(xs,dim=2)
        x = x.view((b,self.n_features*self.embed_size))
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        drop = self.dropout(h)
        logits = torch.matmul(drop,self.hidden_to_logits_weight)+self.hidden_to_logits_bias
