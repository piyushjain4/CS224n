...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        truncated_len = min(truncated_len,length)
        truncated_docu = document[:truncated_len]
        random_no = random.randint(0,int(3*len(truncated_docu)/4))
        prefix = truncated_docu[:random_no]
        normal_no = int(np.random.randn() + len(truncated_docu)/4)
        masked_content = truncated_docu[random_no:random_no+normal_no]
        suffix = truncated_docu[random_no+normal_no:]
