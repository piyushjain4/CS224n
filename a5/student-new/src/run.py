...
if __name__ == '__main__':
    ...
    mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,atten_mecha = 'vanilla',
    ...
    if args.variant == 'vanilla':
        ...
        syn_mconf = model.GPTConfig(pretrain_dataset.vocab_size, pretrain_dataset.block_size,
                    n_layer=4, n_head=8, n_embd=256,atten_mecha = 'synthesizer') 
        model = model.GPT(syn_mconf)
    ...
    if args.function == 'pretrain':
        ...
        tconf = trainer.TrainerConfig(max_epochs=650, batch_size=128, learning_rate=6e-3,
                        ...
                        num_workers=4)
        trainr = trainer.Trainer(model,pretrain_dataset, None, tconf)
        trainr.train()
        ...
        if args.reading_params_path is not None :
            ...
            tconf = trainer.TrainerConfig(max_epochs=10, batch_size=256, learning_rate=6e-4,
                        lr_decay=True, warmup_tokens=512*20, final_tokens=200*len(pretrain_dataset)*block_size,
                        num_workers=4)
            trainr = trainer.Trainer(model,finetune_dataset, None, tconf)
            trainr.train()
        else:
