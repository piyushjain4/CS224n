...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    value = outsideVectors*centerWordVec[None,:]
    value = value.sum(axis=1)
    value = softmax(value)
    loss = -np.log(value[outsideWordIdx])
    gradCenterVec = -(outsideVectors[outsideWordIdx] - np.sum((value[:,None]*outsideVectors),axis=0))
    gradOutsideVecs = value[:,None]*centerWordVec[None,:]
...
def negSamplingLossAndGradient(
    ...
    uniq = np.unique(negSampleWordIndices)
    ## YOUR CODE HERE (~10 Lines)
    value = outsideVectors*centerWordVec[None,:]
    value = value.sum(axis=1)
    val_uniq = value[uniq]
    count_arr = np.bincount(negSampleWordIndices)
    loss = -np.log(sigmoid(value[outsideWordIdx])) - np.sum(count_arr[uniq]*np.log(sigmoid(-val_uniq)))
    gradCenterVec =(sigmoid(value[outsideWordIdx])-1)*outsideVectors[outsideWordIdx] - np.sum(count_arr[uniq][:,None]*((sigmoid(-val_uniq)[:,None]-1)*(outsideVectors[uniq])),axis=0)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(value[outsideWordIdx])-1)*centerWordVec
    gradOutsideVecs[uniq] += count_arr[uniq][:,None]*((1-(sigmoid(-val_uniq)[:,None]))*centerWordVec[None,:])
    ...
    # req_val = np.take_along_axis(value,indices,axis=0)
    # loss = -np.log(sigmoid(value[outsideWordIdx])) - np.sum(np.log(sigmoid(-req_val)))
    # # gradCenterVec +=(sigmoid(value[outsideWordIdx])-1)*outsideVectors[outsideWordIdx] - np.sum(((sigmoid(-req_val)[:,None]-1)*(outsideVectors[indices])),axis=0)
    # gradOutsideVecs[outsideWordIdx] += (sigmoid(value[outsideWordIdx])-1)*centerWordVec
    # gradOutsideVecs[indices] += ((1-(sigmoid(-req_val)[:,None]))*centerWordVec[None,:])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords :
        los,grad_cent,grad_out = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord]],word2Ind[w],outsideVectors,dataset)
        loss +=los
        gradCenterVecs[word2Ind[currentCenterWord]]+=grad_cent
        gradOutsideVectors+=grad_out
    ### END YOUR CODE
