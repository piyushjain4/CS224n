...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(self.embed_size+self.hidden_size,self.hidden_size,bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        output,(hn,cn) = self.encoder(packed_input)
        enc_hiddens,lengths = pad_packed_sequence(output,batch_first=True)
        # last_hidden = torch.cat((hn[0],hn[1]),)
        hn = torch.permute(hn,(1,0,2))
        cn = torch.permute(cn,(1,0,2))
        hn = hn.reshape(source_padded.shape[1],-1)
        cn = cn.reshape(source_padded.shape[1],-1)
        init_decoder_hidden = self.h_projection(hn)
        init_decoder_cell = self.c_projection(cn)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for yt in y_split:
            yt = torch.squeeze(yt)
            y_bar = torch.cat((yt,o_prev),1)
            (dec_state,ot,_) = self.step(y_bar,dec_state,enc_hiddens,enc_hidden_proj,enc_masks) 
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden_3d  = dec_hidden.unsqueeze(2)
        ...
        enc_hiddens = enc_hiddens.transpose(1,2)
        alpha_t_3d = alpha_t.unsqueeze(2)
        a_t = torch.bmm(enc_hiddens,alpha_t_3d)
        a_t = a_t.squeeze(2)
